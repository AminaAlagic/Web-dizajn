    ...
    resultFoodsHtml += `
        <div class="card mx-2 my-2" style="width: 18rem;">
            ...
            <div class="card-body">
                ...
                <button type="button"class="btn btn-primary"  onclick="deleteTodo(${food.id})">Delete</button>
            </div>
        </div>
        `;
  });
...
const filEditData = () => {
  const foodFormId = document.getElementById("id");
  const foodFormName = document.getElementById("name");
  const foodFormImage = document.getElementById("image");
  const foodFormPrice = document.getElementById("price");
  fetch(`${BASE_URL}/api/Food`, {
    method: "POST",
    headers: new Headers({ "content-type": "application/json" }),
    body: JSON.stringify({
      id: foodFormId.value,
      name: foodFormName.value,
      price: foodFormPrice.value,
      imageUrl: foodFormImage.value,
    }),
  }).then((response) => {
